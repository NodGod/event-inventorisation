...
const EventListView = () => {
  ...
  useEffect(() => {
    ...
    const deleteEvent = async (e: SyntheticEvent) => {
        e.preventDefault();
        if (selectedEvent) {
            const URL = '/api/events/' + selectedEvent.id.toString();
    ...
    const createEvent = async(e: SyntheticEvent) => {
        e.preventDefault();
        const response = await axios.post('/api/events', ({name: newEvent.name, description: newEvent.description, date: newEvent.date, address: newEvent.address, organiserId: newEvent.organiserId}))
        console.log(response);
